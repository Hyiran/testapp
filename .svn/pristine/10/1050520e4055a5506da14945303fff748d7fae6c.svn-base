...
public class TestSqlHelper {
	public static SimpleDateFormat tf = new SimpleDateFormat("yyyyMM");//访问每月表的后缀时需要
    ...
    public static void testInsertSpeed(String[] parameters) {
        String sql = "insert into `tb_app_net_"+tf.format(new Date())+"` (productName,networkType,nowTime,down,up,groupId) "
        		+ "values(?,?,?,?,?,?);";
        SqlHelper.executeUpdate(sql, parameters);
        updateLog(parameters[0],"speed",parameters[1]);
    }
	...
	public static void sqlInsertDelay(String[] parameters) {
		...
		String sql = "INSERT INTO `tb_app_data_"+tf.format(new Date())+"` (productName,networkType,nowTime,logintime,receivetime,"
				+ "readtime,sendtime,downtime,groupid)"
				+ "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?);";
		SqlHelper.executeUpdate(sql, parameters);
		//更新执行状态
		updateLog(parameters[0],"data",parameters[1]);
	}
	...
	public static void sqlDataProcessing(){
		...
		sqlFilterSpeed.append("INSERT INTO `tb_app_net_filter_"+tf.format(new Date())+"` (productName,networkType,nowTime,down,up,groupid)\r\n");
		...
		sqlFilterSpeed.append("(select groupId,networkType,(STDDEV_SAMP(down) + STDDEV_SAMP(up))/2 as avgDEV from tb_app_net_"+tf.format(new Date())+"\r\n");
		...
		sqlFilterSpeed.append("left join tb_app_net_"+tf.format(new Date())+" B\r\n");
		...
		sqlFilterData.append("INSERT INTO `tb_app_data_filter_"+tf.format(new Date())+"` (productName,networkType,nowTime,logintime,receivetime\r\n");
		...
		sqlFilterData.append(",BB.readtime,BB.sendtime,BB.downtime,BB.groupid from tb_app_net_filter_"+tf.format(new Date())+" AA\r\n");
		sqlFilterData.append("left join tb_app_data_"+tf.format(new Date())+" BB\r\n");
		...
		sqlResultData.append("INSERT INTO `tb_app_data_result_"+tf.format(new Date())+"` (productName,networkType,nowTime,logintime,receivetime\r\n");
		...
		sqlResultData.append("from tb_app_data_filter_"+tf.format(new Date())+" AA\r\n");
		...
		sqlResultData.append("avg(sendtime) sendtime,avg(downtime) downtime from tb_app_data_filter_"+tf.format(new Date())+"\r\n");
	...
	public static void sqlInsertRate(String[] parameters){
		 String sql = "INSERT INTO `tb_app_rate_result_"+tf.format(new Date())+"` (productName,networkType,nowTime,caseName,time,status,groupid)"
				+ "VALUES ( ?, ?, ?, ?, ?, ?, ?);";
		 SqlHelper.executeUpdate(sql, parameters);
	}
	...
	public static void sqlInsertLoadFlow(String[] parameters){
		 String sql = "INSERT INTO `tb_app_flow_result_"+tf.format(new Date())+"` (productName,networkType,nowTime,loadflow,groupid)"
					+ "VALUES ( ?, ?, ?, ?, ?);";
			 SqlHelper.executeUpdate(sql, parameters);
	}
	...
	public static void sqlUpdateBrushFlow(String[] parameters){
		 String sql = "UPDATE `tb_app_flow_result_"+tf.format(new Date())+"` set brushflow = ? where  networkType =? and groupId = ?";
		 SqlHelper.executeUpdate(sql, parameters);
	}
	...
	public static void sqlInsertStandby(String[] parameters){
		String sql = "INSERT INTO `tb_app_standy_result_"+tf.format(new Date())+"` (productName,networkType,nowTime,caseName,resource,groupid)"
				+ "VALUES ( ?, ?, ?, ?, ?,?);";
		 SqlHelper.executeUpdate(sql, parameters);
	}
	...
	public static ResultSet sqlQuerySpeed(String[] parameters,String tableName){
		...
		sqlQuerySpeed.append("left join \r\n");
		sqlQuerySpeed.append("(select groupId,STDDEV_SAMP(down) as stddown,STDDEV_SAMP(up) as stdup,(STDDEV_SAMP(down) + STDDEV_SAMP(up))/2 as avgDEV from "+tableName+" where networkType = ? group by groupId) EE\r\n");
	...
	public static ResultSet sqlQueryData(String tableName,String productName,String networkType){
		...
		sqlQueryData.append("(select @ROWNO1 := @ROWNO1 + 1 AS ROWNO,D.* from\r\n");
		sqlQueryData.append("(\r\n");
		
		sqlQueryData.append("select groupid,logintime,receivetime,readtime,sendtime,downtime from "+tableName+" AA ,\r\n");
		sqlQueryData.append("(SELECT @ROWNO1 := 0) T3\r\n");
		sqlQueryData.append("where AA.productName = '"+productName+"' \r\n");
		sqlQueryData.append("and AA.networkType = '"+networkType+"' order by groupid) D \r\n");
		...
		sqlQueryData.append("from tb_app_rate_result_"+tf.format(new Date())+" AA\r\n");
		sqlQueryData.append("where AA.productName = '"+productName+"'  \r\n");
		...
		sqlQueryData.append("(\r\n");
		sqlQueryData.append("select loadflow,ABS(loadflow-avg) abs from tb_app_flow_result_"+tf.format(new Date())+" AA\r\n");
		sqlQueryData.append("left join \r\n");
		sqlQueryData.append("(select productName,avg(loadflow) as avg from tb_app_flow_result_"+tf.format(new Date())+" \r\n");
		...
		sqlQueryData.append("(\r\n");
		sqlQueryData.append("select brushflow,ABS(brushflow-avg) abs from tb_app_flow_result_"+tf.format(new Date())+" AA\r\n");
		sqlQueryData.append("left join \r\n");
		sqlQueryData.append("(select productName,avg(brushflow) as avg from tb_app_flow_result_"+tf.format(new Date())+"\r\n"); 
		sqlQueryData.append("where productName = '"+productName+"'  and networkType = '"+networkType+"') BB on (AA.productName = BB.productName)\r\n");
		sqlQueryData.append("where AA.productName = '"+productName+"'  and AA.networkType = '"+networkType+"'\r\n");
		sqlQueryData.append("order by abs\r\n");
		sqlQueryData.append("limit 10) FF,\r\n");
		...
		sqlQueryData.append("(\r\n");
		sqlQueryData.append("select resource,ABS(resource-avg) abs from tb_app_standy_result_"+tf.format(new Date())+" AA\r\n");
		sqlQueryData.append("left join \r\n");
		sqlQueryData.append("(select productName,avg(resource) as avg from tb_app_standy_result_"+tf.format(new Date())+" \r\n");
	 ...
	 * @throws SQLException
	 */
	public static boolean isExistOrder(String month,String networkType) throws SQLException{
		boolean isExist = false;
		String sqlExist = "select count(1)  as isExist from tb_app_order where month = ? and networkType = ?";
		String[] parameters = { month,networkType};
	 ...
	 * @param networkType
	 */
	public static void  insertOrder(String month,String networkType){
		String insert = "insert into tb_app_order (orderNo,month,networkType) VALUES('0',?,?)";
		String[] parameters = { month,networkType};
 ...
 * @throws SQLException
 */
	public static int getOrder(String month,String networkType) throws NumberFormatException, SQLException{
		int groupId = 0;
		String sql = "select orderNo from tb_app_order where month = ? and networkType =? limit 1";
		String[] parameters = { month,networkType};
	 ...
	 * @param networkType
	 */
	public static void updateOrder(int groupId,String month,String networkType){
		String sql = "update tb_app_order set orderno =  ? where month = ? and networkType = ?";
		String[] parameters = {String.valueOf(groupId), month,networkType};
		SqlHelper.executeUpdate(sql, parameters);
	}
	 ...
	 * @param netwokType
	 */
	public static void insertLog(String pcName,int groupId,String netwokType){
		String sql = "insert into tb_app_log(pcName,groupId,speed139,data139,speed163,data163,speedqq,dataqq,speed189,data189,networkType) VALUES (?,?,'N','N','N','N','N','N','N','N',?);";
		String[] parameters = {pcName,String.valueOf(groupId),netwokType};
		SqlHelper.executeUpdate(sql, parameters);
	}
	 ...
	 * @throws SQLException
	 */
	public static Status noSuccess(String pcName,String networkType) throws SQLException{
		Status status = new Status();
		List<String> statusList = new ArrayList<String>();
		status.setPcName(pcName);
		String sql = "select * from tb_app_log where pcName= ? and networkType =?  order by id desc LIMIT 1";
		String[] parameters = {pcName,networkType};
		ResultSet rs = SqlHelper.executeQuery(sql, parameters);
		while(rs.next()){
			...
			status.setSpeed189(rs.getString(10).toString());
		...
		statusList.add(status.getData189());
		//如果不包含,则为
	 ...
	 * @param networkType
	 */
	public static void updateLog(String productName,String type,String networkType){
		String field = "";
		...
		String sql = "update tb_app_log set "+field+" = 'Y' where pcName = ? and networkType = ? order by id desc LIMIT 1";
		String[] parameters = {CommonUtil.getName(),networkType};
		SqlHelper.executeUpdate(sql, parameters);
	}
    ...
    public void testInsertMysql() {
        String sql = "insert into `tb_web_data_"+tf.format(new Date())+"` (productName,networkType,case1,"+
